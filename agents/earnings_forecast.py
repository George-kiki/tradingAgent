...
def _is_recent_period(end_date: str, max_age_days: int = 540) -> bool:
    """业绩预告时效过滤：过老预告只做历史信息，不参与前瞻推演。"""
    try:
        d = dt.datetime.strptime(str(end_date)[:8], "%Y%m%d").date()
    except Exception:
        return False
    return d >= (dt.date.today() - dt.timedelta(days=max_age_days))
...
def _self_forecast(fetcher, symbol: str) -> dict:
    """本公司近期官方业绩预告 + 快报。过期预告不参与前瞻推演。"""
    raw_fc = fetcher.get_forecast(symbol) or []
    fc = [x for x in raw_fc if _is_recent_period(x.get("end_date", ""))]
    ex = fetcher.get_express(symbol) or []
    latest = fc[0] if fc else None
    stale_latest = raw_fc[0] if raw_fc and not fc else None
    note = ""
    stale_note = ""
    if latest:
        ...
        stale_note = f"最近一条官方预告为 {str(stale_latest.get('end_date',''))[:6]}，已过期，不参与当前业绩前瞻。"
    return {
        "forecasts": fc,
        "raw_forecasts": raw_fc,
        "express": ex,
        "latest": latest,
...
def _chain_prosperity(fetcher, symbol: str, industry: Optional[str]) -> dict:
    ...
    try:
        ...
        for code in codes:
            if code == symbol:
                continue
            fc = [x for x in (fetcher.get_forecast(code) or []) if _is_recent_period(x.get("end_date", ""))]
            if not fc:
                continue
            latest = fc[0]
            t = latest.get("type", "")
